...
function routeGallery(req, res) {
  try {
    ...
    res.render(process.env.VIEW_GALLERY, { web_title: process.env.WEB_TITLE, web_signature: process.env.WEB_SIGNATURE, prefix_thumbnails: process.env.PREFIX_THUMBNAILS, prefix_previews: process.env.PREFIX_PREVIEWS, photoFiles: photoFilenames, paramStart });
    res.end();
  } catch (error) {
    console.error(error);
